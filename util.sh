...
function detect-root-device {
  ...
  NODE_BLOCK_DEVICE_MAPPINGS="[{\"DeviceName\":\"${ROOT_DEVICE_NODE}\",\"Ebs\":{\"VolumeSize\":${NODE_ROOT_DISK_SIZE}}} ${EPHEMERAL_BLOCK_DEVICE_MAPPINGS}]"
}
...
function upload-server-tars() {
  ...
  local s3_url_base=https://s3.amazonaws.com
  ...
  #aws s3 sync --region ${s3_bucket_location} ${local_dir} "s3://${AWS_S3_BUCKET}/${staging_path}/"
